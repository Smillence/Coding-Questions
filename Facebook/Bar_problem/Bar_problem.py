'''To run the program, enter: python Bar_probrom.py input.txt 

Reference: https://github.com/alonsovidales/facebook-programming-challenges/tree/master/bar
...
In the sample example given, for the first testcse N = 3 and K = 3. List for the first person is {2 5 3} , {8 1 6} for second and {7 4 9} for the third. In this case all the numbers in {4, 5, 6, 7, 8, 9} have a chance to be the third biggest selected number.
...
# loop thoruhg all users and all number
def bruth_force_search(users):
    ...
    paths = []
    for num in user:
        subpaths = bruth_force_search(users)
        for path in subpaths:
            path += [num]
        paths += subpaths
    return paths

#linear select is the best solution in O(n)
def select(lst,k):
    lst.sort()
    return lst[k-1]
...
if __name__ == '__main__':    
    ...
    for t in range(T):
        N,K = txt.readline().split()
        N,K = int(N),int(K)
        users = []

        for n in range(N):
            user = txt.readline().split()[1:]
            user = [int(s) for s in user]
            user.sort()
            users.append(user)

        #1) solution 1 bruth-force (runs for ever)
        '''
        paths = bruth_force_search(users)
        ...
        #2) solution 2
        #say users i chooses number X,
        #find there are a users have options less than X
        #   and b users have options larger than X
        # if a >= K-1 and b >= N-K, we say X is a possible one
        count = 0
        for user in users:
            rest_users = copy.deepcopy(users)
            rest_users.remove(user)
            for num in user:
                has_smaller_count = 0
                has_larger_count = 0
                for otheruser in rest_users:
                    if num > otheruser[0]:
                        has_smaller_count +=1
                    if num < otheruser[-1]:
                        has_larger_count +=1
                if has_smaller_count >= K-1 and has_larger_count >= N-K:
                    count +=1
        print count
